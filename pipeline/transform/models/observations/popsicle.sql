...
SELECT
    ...
    block_number,
    CAST (
        get_inventory.outputs__amount0 AS numeric
    ) AS inventory0,
    CAST (
        get_inventory.outputs__amount1 AS numeric
    ) AS inventory1,
    CAST(
...
FROM
    tap_ethereum.popsicle_getters_totalsupply AS total_supply
    JOIN tap_ethereum.popsicle_getters_usersamounts AS get_inventory USING (
        "address",
        block_number
    )
